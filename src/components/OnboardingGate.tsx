...
export default function OnboardingGate({ slides, children }: Props) {
  ...
  // ═══ SLIDES ═══ — Extrahiert in components/onboarding/SlidesPanel.tsx
  return (
    <SlidesPanel
      slides={slides}
      step={step}
      onStepChange={setStep}
      onLastSlideContinue={() => setPhase('roleSelect')}
      onSkip={() => finish('CUSTOMER')}
      labels={{
        back: t('onboarding.back'),
        next: t('onboarding.next'),
        letsGo: t('onboarding.letsGo'),
        skipDiscover: t('onboarding.skipDiscover'),
      }}
    />
  )
}
