        ...
        <div style={{ width: '100%', display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
          {provCategories.map(cat => (
            <button key={cat.id} onClick={() => {
              ...
              boxShadow: provCat === cat.id ? '0 0 28px rgba(190,133,16,.1)' : undefined,
              position: 'relative',
            }}>
              {provCat === cat.id && (
          ...
          {allServices.map(s => {
            ...
            return (
              <button key={s.nm} onClick={() => toggleSvc(s.nm)} style={{
                ...
                <div style={{ width: 20, height: 20, borderRadius: 4, border: '1.5px solid var(--gold)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: 'var(--gold)', background: active ? 'rgba(190,133,16,.15)' : 'transparent', flexShrink: 0 }}>
                  {active ? '✓' : ''}
                </div>
                <span style={{ flex: 1, fontSize: 13, fontWeight: active ? 700 : 400, color: active ? 'var(--cream)' : 'var(--stone)' }}>
                  {s.nm} {isCustom && <span style={{ fontSize: 9, padding: '2px 5px', borderRadius: 4, background: 'rgba(190,133,16,.1)', color: 'var(--gold2)', marginLeft: 4 }}>EIGENER</span>}
                </span>
              ...
              {allEquip.map(eq => {
                ...
                return (
                  <button key={eq.nm} onClick={() => toggleEq(eq.nm)} style={{
                    ...
                    <span style={{ flex: 1, fontSize: 13, fontWeight: active ? 700 : 400, color: active ? 'var(--cream)' : 'var(--stone)' }}>
                      {eq.nm} {isCustom && <span style={{ fontSize: 9, padding: '2px 5px', borderRadius: 4, background: 'rgba(190,133,16,.1)', color: 'var(--gold2)', marginLeft: 4 }}>EIGENER</span>}
                    </span>
    ...
    if (provStep === 5) {
      ...
      return shell(<>
        ...
        <button onClick={() => setAgb(!agb)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: 12, background: 'var(--c2)', borderRadius: 12, border: agb ? '1px solid var(--gold)' : '1px solid var(--border)', cursor: 'pointer', marginBottom: 20 }}>
          <div style={{ width: 22, height: 22, borderRadius: 5, border: '1.5px solid var(--gold)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: 'var(--gold)', background: agb ? 'rgba(190,133,16,.15)' : 'transparent', flexShrink: 0 }}>
            {agb ? '✓' : ''}
          </div>
      ...
      <div id="ob-scroll" style={{
        ...
        {isFirst ? (
          ...
          <div style={{
            ...
            height: 96,
            borderRadius: 28,
            border: '1.5px solid rgba(190,133,16,0.2)',
            background: 'rgba(190,133,16,0.05)',
            display: 'inline-flex',
            alignItems: 'center',
            justifyContent: 'center',
          }}>
            {(slide.imageUrl?.includes('termin') || slide.icon === 'booking') ? (
              ...
              <svg width="44" height="44" viewBox="0 0 24 24" fill="none" stroke="var(--gold)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                ...
                <polygon points="16.24,7.76 14.12,14.12 7.76,16.24 9.88,9.88" fill="rgba(190,133,16,0.15)" stroke="var(--gold)" />
              </svg>
            )}
          </div>
        )}
