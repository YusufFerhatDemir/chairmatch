          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
              ...
              <span className="badge" style={{
                fontSize: 9,
                background: s.status === 'approved' ? 'rgba(74,138,90,0.15)' : s.status === 'pending' ? 'rgba(190,133,16,0.1)' : 'rgba(232,80,64,0.1)',
                color: s.status === 'approved' ? '#6ABF80' : s.status === 'pending' ? 'var(--gold)' : 'var(--red)',
                border: `1px solid ${s.status === 'approved' ? 'rgba(74,138,90,0.3)' : s.status === 'pending' ? 'rgba(190,133,16,0.2)' : 'rgba(232,80,64,0.2)'}`,
