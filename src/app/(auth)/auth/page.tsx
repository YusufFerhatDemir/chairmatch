// V12 deployed 2026-05-20 — TR-i18n + Kreise weg
'use client'
 ...
 *
 * Alle Strings über `t('welcomeSplitter.xxx')` — i18n-fähig (DE/EN/TR/AR).
 */
interface RoleCard {
...
const ROLES: RoleCard[] = [
  {
    id: 'kunde',
    icon: CalendarCheck,
    titleKey: 'welcomeSplitter.roleKundeTitle',
    subtitleKey: 'welcomeSplitter.kundeRowSub',
    href: '/explore',
  },
  {
    id: 'anbieter',
      ...
      <div className="screen" style={{ padding: 'var(--pad)', display: 'flex', flexDirection: 'column', justifyContent: 'center', minHeight: '80vh' }}>
        ...
        >
          ...
          <span className="cinzel" style={{
            ...
            {t('welcomeSplitter.orStartDirect')}
          </span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {ROLES.map((role) => {
            ...
            const subtitle = t(role.subtitleKey)
            return (
              <Link
                key={role.id}
              ...
              >
                ...
                {/* Kreis › entfernt — saubere Cards (User-Request 2026-05-20) */}
              </Link>
            )
          })}
        </div>

        {/* Escape-Hatch — Kreis › entfernt */}
        <Link
          href="/"
          style={{
            display: 'flex', alignItems: 'center', justifyContent: 'center',
        ...
        >
          {[
            { Icon: ShieldCheck, title: t('welcomeSplitter.trustSafeTitle'),     subtitle: t('welcomeSplitter.trustSafeSub') },
            { Icon: BadgeCheck,  title: t('welcomeSplitter.trustVerifiedTitle'), subtitle: t('welcomeSplitter.trustVerifiedSub') },
            { Icon: Building2,   title: t('welcomeSplitter.trustFlexTitle'),     subtitle: t('welcomeSplitter.trustFlexSub') },
          ].map((b, idx) => (
            <div
              key={b.title}
              style={{
        ...
        <div style={{ maxWidth: 520, width: '100%' }}>
          ...
          <div style={{
            ...
            {SPLITTER_ROLES.map((role) => {
              const title = t(role.titleKey)
              const sub1 = t(role.sub1Key)
              const sub2 = t(role.sub2Key)
              return (
                <Link
                  key={role.id}
                  href={role.href as never}
                ...
                >
                  ...
                  {/* Pfeil-Kreis › entfernt — saubere Cards (User-Request 2026-05-20) */}
                </Link>
              )
            })}
          </div>

          {/* Ohne Anmeldung entdecken — Kreis › entfernt */}
          <Link
            href="/"
            style={{
              width: '100%',
