...
## Standard-Tools für Agents

- `./deploy.sh "msg"` — staged + pre-commit-guard + typecheck + commit + push **current branch** + verify-push + updated STATUS.md
- `./scripts/rollback.sh [N]` — revertiert die letzten N Commits sauber via `git revert` (kein force-push)
- `./scripts/status.sh` — generiert `STATUS.md` mit echtem git/Vercel-Stand
- `./scripts/verify-push.sh` — prüft ob lokaler HEAD wirklich auf origin ist (gleicher Branch)
- `./scripts/precommit-guard.sh` — blockt `.env`, `node_modules`, Secret-Patterns (Resend/Stripe/AWS/JWT)

**Regeln:**
- Agents müssen `./deploy.sh` nutzen, nicht direkt `git push`.
- Agents dürfen NICHT "gepusht" claimen wenn `verify-push.sh` durchfällt — das ist die einzige Wahrheit.
- `STATUS.md` ist nie manuell editieren — wird von `scripts/status.sh` regeneriert.
